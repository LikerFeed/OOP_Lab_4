...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
        ...
        IvanToolBar.OnTool(hWnd, ID_TOOL_POINT); //selecting point from start
    ...
    case WM_COMMAND:
    {
        ...
        case ID_TOOL_POINT:
        case ID_32771:
            IvanToolBar.OnTool(hWnd, ID_TOOL_POINT);
            if (myeditor) myeditor->Start(hWnd, new PointShape);
            break;

        case ID_TOOL_LINE:
        case ID_32772:
            IvanToolBar.OnTool(hWnd, ID_TOOL_LINE);
            if (myeditor) myeditor->Start(hWnd, new LineShape);
            break;

        case ID_TOOL_RECT:
        case ID_32773:
            IvanToolBar.OnTool(hWnd, ID_TOOL_RECT);
            if (myeditor) myeditor->Start(hWnd, new RectShape);
            break;

        case ID_TOOL_ELLIPSE:
        case ID_32774:
            IvanToolBar.OnTool(hWnd, ID_TOOL_ELLIPSE);
            if (myeditor) myeditor->Start(hWnd, new EllipseShape);
            break;

        case ID_TOOL_POINTPLUS:
        case ID_32775:
            IvanToolBar.OnTool(hWnd, ID_TOOL_POINTPLUS);
            if (myeditor) myeditor->Start(hWnd, new PointPlusShape);
            break;

        case ID_TOOL_CUBE:
        case ID_32776:
            IvanToolBar.OnTool(hWnd, ID_TOOL_CUBE);
            if (myeditor) myeditor->Start(hWnd, new CubeShape);
            break;

        case ID_TOOL_LINEOO:
        case ID_32777:
            IvanToolBar.OnTool(hWnd, ID_TOOL_LINEOO);
